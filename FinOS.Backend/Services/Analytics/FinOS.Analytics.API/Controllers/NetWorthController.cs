...
[Authorize]
public class NetWorthController : AuthenticatedControllerBase
{
    ...
    [HttpGet("trend")]
    public async Task<ActionResult<ApiResponse<List<NetWorthDto>>>> GetTrend([FromQuery] int months = 12)
    {
        var result = await _mediator.Send(new GetNetWorthTrendQuery(AuthenticatedUserId, months));
    ...
    {
        var result = await _mediator.Send(new CalculateNetWorthCommand(dto with { UserId = AuthenticatedUserId }));
