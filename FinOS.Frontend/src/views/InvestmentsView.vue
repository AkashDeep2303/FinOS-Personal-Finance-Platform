...
const tabs=[{id:'portfolio',label:'Portfolio'},{id:'sips',label:'SIP Tracker'},{id:'epf',label:'EPF Tracker'}]
...
onMounted(async()=>{await Promise.allSettled([store.fetchInvestments(),store.fetchSIPs(),store.fetchEPF(),accounts.fetchAccounts()]);if(store.epfTracker)contribution.monthlySalary=store.epfTracker.monthlySalary})
</script>
<style scoped>
